...
public class IscreateRevokeTaskCmd implements Command<Boolean> {

	private String taskId;
	private String userId;
	
	public IscreateRevokeTaskCmd(){
		
	}
	
	public IscreateRevokeTaskCmd(String taskId,String userId){
		this.taskId=taskId;
		this.userId=userId;
	}
...
//		ProcessInstance processInstance=pe.getExecutionService().createProcessInstanceQuery().processInstanceId(piid).uniqueResult();
		...
		action.setActionContent(userId+" 撤销任务:"+taskId);
		action.setActionName("撤销");
		action.setActionType(ProcessConstants.TaskActionType.CANCEL_TASK);
