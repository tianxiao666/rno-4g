...
import com.iscreate.op.service.rno.parser.jobrunnable.Hw2GMrrParserJobRunnable.DBFieldToTitle;
...
//		status.setJobRunningStatus(JobRunningStatus.Running);
		...
		//获取小区与小区id映射
		String lteCellSql = "select business_cell_id, pci,earfcn  from rno_lte_cell  where area_id in("+areaIdStr+")";
...
//			}
		...
		try {
			...
			context.addHtable("descTable","LTE4G_HO_DESC",conf);
			context.addHtable("dataTable","LTE4G_ZTE_HO",conf);
		...
		try{
			...
			do {
				...
				if(cellIdToBcch.get(ncellIdStr[3]+ncellIdStr[4]) == null) {
					//在小区工参匹配不出小区名
					log.debug("在小区工参匹配不出小区名："+ncellIdStr[3]+ncellIdStr[4]+"，该行数据不入库");
					continue;
				}
