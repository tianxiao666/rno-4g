...
public class G4DtSweepFreqParserJobRunnable extends DbParserBaseJobRunnable {

	private static Log log = LogFactory.getLog(G4DtSweepFreqParserJobRunnable.class);
	
	public G4DtSweepFreqParserJobRunnable() {
		super();
		super.setJobType("4GDTSWEEPFREQFILE");
	}
	//数据表
...
//		charset = FileTool.getFileEncode(f.getAbsolutePath());
		...
		try {
			...
			log.debug("DT文件：" + tmpFileName + ",title 为：" + line + ",有"
					+ fieldCnt + "标题");
			if(line.contains("APP Speed (Mbps)")){
				log.error("检查DT文件：" + tmpFileName + "的标题头有问题！" + msg);
				date2 = new Date();
				msg = "APP Speed (Mbps)";
				report.setFields("检查文件标题", date1, date2,
						DataParseStatus.Failall.toString(), "在文件中多出以下标题:" + msg);
				addJobReport(report);
				// connection.releaseSavepoint(savepoint);
				return false;
			}
...
//								System.out.println("sp==" + sp+"---dtf=="+dtf);
			...
			ws += cityId + ",'SweepFreq',SEQ_RNO_4G_DT_SAMPLING.nextval,?";
			// 此处也要增加相应的位置索引
...
//				System.out.println("line====>>>>"+line);
				//为了标题与内容不错位，进行以下替换
				lineBefor=line.substring(0, line.indexOf(")")+1);
				lineAfter=line.substring(line.indexOf(")")+1);
				if(lineBefor.contains(",")){
					lineBefor=lineBefor.replace(",", "-").replace(" ", "");
				}
				line=lineBefor+lineAfter;
				sps = line.split(",|\t");
				totalDataNum++;
...
//			ResultInfo resultInfo = new ResultInfo();
			...
			DtObj dtObj=null;
			boolean flag=true;
			String insertDescSql="insert into "+descTable+" (dt_sampling_desc_id,file_name,mea_time,record_count,create_time,mod_time,city_id,data_type,area_type) values(?,?,?,?,sysdate,sysdate,"+cityId+",'SweepFreq','"+areaType+"')";
	...
	private static Map<String, DBFieldToTitle> readDbToTitleCfgFromProfile() {
		...
		try {
			...
			is = new BufferedInputStream(new FileInputStream(
					G4DtSweepFreqParserJobRunnable.class.getResource(
	...
	public Map<String, DBFieldToTitle> readDbToTitleCfgFromXml() {
		...
		try {
			InputStream in = new FileInputStream(new File(
					G4DtSweepFreqParserJobRunnable.class.getResource(
							"g4DtSweepFreqDbToTitles.xml").getPath()));
...
//		dateUtil.to_yyyyMMddDate("15-06-01 10:02:01.793");
		...
		String aa="FileName,Time,Longitude,Latitude,CellName,TAC,ECI,EARFCN,PCI,RSRP,RS SINR,SCell Dist(km),SCell Azimuth Diff,APP Speed (Mbps),NCell Name #1,NCell EARFCN #1,NCell PCI #1,NCell RSRP #1,NCell Name #2,NCell EARFCN #2,NCell PCI #2,NCell RSRP #2,NCell Name #3,NCell EARFCN #3,NCell PCI #3,NCell RSRP #3,NCell Name #4,NCell EARFCN #4,NCell PCI #4,NCell RSRP #4,NCell Name #5,NCell EARFCN #5,NCell PCI #5,NCell RSRP #5,NCell Name #6,NCell EARFCN #6,NCell PCI #6,NCell RSRP #6,";
		System.out.println(aa.contains("APP Speed (Mbps)"));
		System.out.println(aa.equals("APP Speed (Mbps)"));
	}
