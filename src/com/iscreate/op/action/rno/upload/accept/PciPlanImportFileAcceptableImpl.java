...
public class PciPlanImportFileAcceptableImpl extends BaseFileAcceptableImpl{

	private static Log log=LogFactory.getLog(PciPlanImportFileAcceptableImpl.class);
	@Override
	public FileAcceptStatus isAcceptable(long size, String dataType,
		...
		if(!status.isFlag()){
			...
			SessionService.getInstance().saveSession("session", ServletActionContext.getRequest()
					.getSession());
			SessionService.getInstance().setValueByKey("pciCell", attmsg.get("pciCell"));
		}
		//判断是否有相应的日期的小区数据
		//如果没有，需要拒绝
