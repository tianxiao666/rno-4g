...
.mF_classicHC .pic{border:0px solid #666;overflow:hidden;}
...
.mF_classicHC .num-bg{width:100%;background:url(../../home/showcase/ImagePlayer/img/photo_bg.png) right bottom no-repeat;_height:16px;_background:0;_filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='../../home/showcase/ImagePlayer/img/photo_bg3.png', sizingMethod='scale');}
...
.mF_classicHC .num li.current,.mF_classicHC .num li.hover{background:url(../../home/showcase/ImagePlayer/img/photo_bg2.gif) left top no-repeat;}/*当前/悬停按钮样式*/
...
myFocus.setting({
	...
	width:205,//宽(大图)，注意:整个焦点图的宽等于或大于图片宽
	height:150//高(大图)，注意同上
});


$(function(){
	var myurl ="getContentPictureByChannelId.action";
	...
	$.ajax({
           ...
           success : function (result){
				...
				$.each(result, function(key, value) {
					if(key<5){
						photoHTML +='<li><a href="'+value.originUrl+'" target="blank_"><img src="../../'+value.picturePath+'" thumb="" alt="'+value.pictureDesc+'" text="" /></a></li>';
					}
					//alert(value.contentId);
				});
				$("#picture_ul").html(photoHTML);
           }
       });
...
<body  style="margin:0px auto;text-align:center;padding:0px 0 0 0px;font-size:12px;">
<div id="myFocus">
	<div class="loading"><span>请稍候...</span></div><%--载入画面--%>
	<ul class="pic" id="picture_ul"><%--内容列表--%>
