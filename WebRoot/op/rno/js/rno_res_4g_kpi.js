...
function bindEvent(){
	$("#importBtn").click(function() {
		...
		$("#err").remove();
		if ($("#fileid").val() == "") {
			$("#fileid").parent().append("<font id='err' color='red'>请选择KPI文件</font>");
			return;
		}
		if ($.trim($("#meatime").val()) == "") {
...
//				timeFormat: "HH:mm:ss",
				...
				numberOfMonths : 1,
				onClose : function(selectedDate) {
					$("#kpiMeaEndDate").datepicker("option", "minDate",
							selectedDate);
				}
			});
	$("#kpiMeaBegDate").datepicker("setDate",addDays(new Date(),-2));// 减去2天
	$("#kpiMeaEndDate").datepicker(
			{
				dateFormat : "yy-mm-dd",
//				timeFormat: "HH:mm:ss",
				defaultDate :"+1w",
				...
				numberOfMonths : 1,
				onClose : function(selectedDate) {
					$("#kpiMeaBegDate").datepicker("option", "maxDate",
							selectedDate);
				}
			});
	$("#kpiMeaEndDate").datepicker("setDate",(new Date()));
 ...
 * 查询kpi描述数据
 */
function queryKpiDescData(){
	$("#searchMrForm").ajaxSubmit({
		url:'queryKpiDescAjaxAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
 ...
 * @param data
 */
function displayKpiDescData(data){
	if(data==null||data==undefined){
		return;
	}
	//
