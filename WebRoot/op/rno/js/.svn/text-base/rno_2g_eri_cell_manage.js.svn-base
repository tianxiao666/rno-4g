...
function bindEvent(){
	...
	$("#importBtn").click(function() {
		...
		$("#err").remove();
		if ($("#fileid").val() == "") {
			$("#fileid").parent().append("<font id='err' color='red'>请选择爱立信2G小区文件</font>");
			return;
		}
		if ($.trim($("#meatime").val()) == "") {
		...
		if(fileCode=="2GERICELLFILE"){
			//验证文件日期格式
			var fileName=$("#fileid").val();
			var file=fileName.split(" ");
			var fileDate;
			if(file.length!=2){
				$("#fileid").parent().append("<font id='err' color='red'>文件名不符合命名规范，如2014-2-30 xxxx.xxx</font>");
				return;
			}else{
				fileDate=file[0];
				if(!checkDate(fileDate)){
					$("#fileid").parent().append("<font id='err' color='red'>日期格式不正确</font>");
					return;
				}
			}
			//验证文件日期与所选日期一致性
			var date1=new Date($.trim($("#meatime").val()));
			var fileDateArr=fileDate.split("-");
			if(fileDateArr[1].length<2){
				fileDate=fileDateArr[0]+"-0"+fileDateArr[1]+"-"+fileDateArr[2];
			}
			var date2=new Date(fileDate);
			if(date1.getTime()!=date2.getTime()){
				$("#fileid").parent().append("<font id='err' color='red'>所选日期与文件标注日期不一致</font>");
				return;
				}
			//判断是否存在数据
			confirmEri2GCellCnt();
...
function displayCellDescData(data){
	...
	for ( var i = 0; i < data.length; i++) {
		...
		html+="<td>"+getValidValue(one['DATA_TYPE']=='CELLDATA'?'小区数据':'工参数据','')+"</td>";
