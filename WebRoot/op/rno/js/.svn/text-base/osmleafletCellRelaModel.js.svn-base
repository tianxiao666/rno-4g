...
//key： 当key不存在默认产生gis小区复合点，key有值就生成其他类型小区复合点，例如lte小区可用‘lcid’做key
function ComposeMarker(gisCell,key) {
	...
	//key不存在默认gis小区，存在则表示为其他类型小区，例如lte小区； 加上“”是为了改为字符串格式
...
//	var alnglats = gisCell.allLngLats;// 113.2,23.33;113.233,23.334;113.222,23.234
	...
	for ( var i = 0; i < alls.length; i++) {
		...
		var point = L.latLng(one[1],one[0]);
...
ComposeMarker.prototype.addCell = function(gisCell,key) {
	...
	if (me._count > 1) {
		...
		//key判断加载的是其他类型的小区还是gis小区，默认gis，如果加载lte小区可以赋值key为‘lcid’
...
// ----标注类-----//
function IsTextLabel(lng, lat, data,opts,eventFunction,osm) {
	this.map=osm.map;
	this.lng = lng;
	this.lat = lat;
	var point = L.latLng(Number(lat),Number(lng));
	this.center = point;
	this.data = data;
	var textlabel=this.createTextLabel(lng,lat,data,osm);
	return textlabel;
	//>>>>>>>>>>>>>>扩展
...
IsTextLabel.prototype.createTextLabel=function(lng,lat,data,osm) {
		var cont = "";
		var redcolor={"fillColor":"#FF0000","fillOpacity":1};
		var yewcolor={"fillColor":"#FFFF00","fillOpacity":1};
		// 创建地标。
		var myIcon = L.icon({
				iconUrl: ' ',
				iconSize: [120, 20]
		...
		var label=L.marker(latlng,{icon:myIcon});
		if (this.data) {
			if (this.data instanceof LabelContent) {
				cont = this.data.getStyleContent();
				if (data.strbcch && data.strbcch != "") {
				label._icon.className=redcolor;
				}else{
				label._icon.className=yewcolor;
				}
			} else {
				cont = data['content'];
...
//				console.log(cont+":"+color);
				...
				label._icon.className=color;
				}
			}
		}
		if("undefined"!=typeof(cont)){
			label._icon.alt(cont);
		}else{
			label._icon.alt(data);
			label._icon.className=yewcolor;
		}
		// 设置地标的位置。
