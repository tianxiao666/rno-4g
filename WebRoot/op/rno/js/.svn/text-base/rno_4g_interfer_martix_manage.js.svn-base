$(document).ready(
	...
	$("#addInterMartix").click(function(){
		location.href = 'init4GInterferMartixAddForAjaxAction';
	});
});
...
function sumbitInterferMartixForm() {
	...
	$("#interferMartixForm").ajaxSubmit({
		url : 'query4GInterferMartixByPageForAjaxAction',
		dataType : 'text',
...
function showInterMartix(raw) {
	...
	for(var i=0;i<list.length;i++){
		...
		downstr="<input type='button' onclick='download4GMatrixFile(\""+jobId+"\")' value='下载结果文件'>";
