...
function bindEvent(){
	$("#importBtn").click(function() {
		...
		$("#err").remove();
		if ($("#fileid").val() == "") {
			$("#fileid").parent().append("<font id='err' color='red'>请选择dt文件</font>");
			return;
		}
		/*if ($.trim($("#meatime").val()) == "") {
...
function doUpload() {
	...
	/*console.log($("#formImportMr #areaType").find("option:selected").val());
	attachParams['areaType'] = $("#formImportMr #areaType").find("option:selected").val();*/
	var areaType=$("#formImportMr #areaType").find("option:selected").val();
	// 询问是否可以上传
	$.ajax({
		...
		data : {
			'fileSize' : fileSize,
			'fileCode' : fileCode,
			'attachParams.areaType' : areaType
		},
		type : 'post',
		dataType : 'json',
...
function displayMrDescData(data){
	...
	for ( var i = 0; i < data.length; i++) {
		...
		html+="<td>"+getValidValue(one['FILE_NAME'],'')+"</td>";
