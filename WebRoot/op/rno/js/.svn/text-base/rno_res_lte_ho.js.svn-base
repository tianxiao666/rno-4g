...
function bindEvent(){
	$("#importBtn").click(function() {
		...
		$("#err").remove();
		if ($("#fileid").val() == "") {
			$("#fileid").parent().append("<font id='err' color='red'>请选择切换文件</font>");
			return;
		}
		if ($.trim($("#meatime").val()) == "") {
	...
	$("#searchImportBtn").click(function(){
		...
		var factory=$("#formImport #factory").find("option:selected").val();
		$("#searchImportDiv #fileCode").val(factory);
		queryImportDataRec();
	});
	
	//查询切换描述数据
	$("#searchHoBtn").click(function(){
		initFormPage("searchHoForm");
		queryHoDescData();
	});
...
function jqueryUiSet() {
	...
	$("#importDiv").css("height","auto");
	$("#formImport").css({"padding":"0px","margin":"0px"});
	...
	$("#hoMeaBegDate").datetimepicker(
			{
				...
				numberOfMonths : 1,
				onClose : function(selectedDate) {
					$("#hoMeaEndDate").datetimepicker("option", "minDate",
							selectedDate);
				}
			});
	$("#hoMeaBegDate").datetimepicker("setDate",addDays(new Date(),-2));// 减去2天
	$("#hoMeaEndDate").datetimepicker(
			{
				dateFormat : "yy-mm-dd",
				timeFormat: "HH:mm:ss",
				defaultDate :"+1w",
				...
				numberOfMonths : 1,
				onClose : function(selectedDate) {
					$("#hoMeaBegDate").datetimepicker("option", "maxDate",
							selectedDate);
				}
			});
	$("#hoMeaEndDate").datetimepicker("setDate",(new Date()));
...
function queryImportDataRec(){
	$("#searchImportForm").ajaxSubmit({
		...
		success:function(raw){
			...
			setPageView(data['page'],"hoImportRecPageDiv");
		}
	});
}
...
function doUpload() {
	...
	var factory=$("#formImport #factory").find("option:selected").val();
	$("#formImport #fileCode").val(factory);
	$("#progressNum").text('0%');
	$("#progressbar").progressbar({
		value : 0
	});
	...
	fileSize = 1;
	var fileCode = $("#formImport #fileCode").val();
 ...
 * 查询切换描述数据
 */
function queryHoDescData(){
	$("#searchHoForm").ajaxSubmit({
		url:'queryHoDescAjaxAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
			...
			displayHoDescData(data['data']);
			setFormPageInfo("searchHoForm",data['page']);
			setPageView(data['page'],"hoListPageDiv");
		}
	});
}
 ...
 * @param data
 */
function displayHoDescData(data){
	if(data==null||data==undefined){
		return;
	}
	//
	$("#hoListTab tr:not(:first)").each(function(i, ele) {
		$(ele).remove();
	});
	var html="";
	...
	for ( var i = 0; i < data.length; i++) {
		one = data[i];
		factory=getValidValue(one['FACTORY'],'');
		if(factory=="ERI"){
			factory="爱立信";
		}else if(factory=="ZTE"){
			factory="中兴";
