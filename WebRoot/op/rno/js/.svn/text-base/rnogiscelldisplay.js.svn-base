...
var randomShowCnt = 500;// 在不需要全部显示的时候，最大随机显示的个数
...
//相似度判断阈值
var DiffAzimuth = 30;// 角度
var DiffDistance = 40;// 距离（米）
...
//		 	console.log("切换后是百度");
	...
	$("#loadltecell").click(function(){
			...
			mapGridLib.loadLteCellBySector(t-minResponseInterval, currentloadtoken, minResponseInterval);
...
function handleClick(polygon, cell) {
	if(!cell) {
	 ...
	 //console.log("关联的小区为：" + cell);
	//showCellDetail(cell);
	showLteCellDetail(cell);
}
//
///**
// * 处理鼠标移动事件
 ...
 * @param label
 */
function showLteCellDetail(label) {
	// alert("关联的小区为：" + polygon._data.getCell());
	...
	$.ajax({
		//url : 'getCellDetailForAjaxAction',
		url : 'getLteCellDetailByBusCellIdForAjaxAction',
		data : {
			'label' : label
		},
		dataType : 'json',
...
function handleMoveendAndZoomend(e, lastOperTime) {
	...
	mapGridLib.loadLteCellBySector(lastOperTime, currentloadtoken, minResponseInterval);
}
