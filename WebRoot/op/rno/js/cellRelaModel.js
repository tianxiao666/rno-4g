...
function ComposeMarker(gisCell,key) {
	...
	this._cellArray = new Array();
	this._polygon = null;
	this._pci = key ?"" : (gisCell.pci ? gisCell.pci : ""); //2015-8-3 cc加入  根据key判断是否加载pci
	
	//2015-7-27 cc加入 话统数据的特殊标记
	//this._thFlag = gisCell.thFlag?gisCell.thFlag:false;
	//2015-8-4 cc修改 配合key进行是否显示三色的判断
	this._thFlag = (typeof gisCell.thFlag!="undefined")?gisCell.thFlag:"";
	// 显示用的经纬度
//	var alnglats = gisCell.allLngLats;// 113.2,23.33;113.233,23.334;113.222,23.234
	/*var lnltlists = gisCell.allLngLats;// 113.2,23.33;113.233,23.334;113.222,23.234
	var data=eval("("+lnltlists+")");
...
ComposeMarker.prototype.getPolygonOptions = function(singleColor, multiColor) {
	...
	//2015-4-22 pjm 加入  满足lte小区三色显示
	if(this._pci != ""&&(this._thFlag==="")) { //2015-8-3 cc加入  满足问题小区的判断，显示问题小区时，不采用三色显示。
...
ComposeMarker.prototype.addCell = function(gisCell) {
	...
	if (me._count > 1) {
		...
		me._chineseName = gisCell.chineseName;
...
ComposeMarker.prototype.addCell = function(gisCell,key) {
	...
	if (me._count > 1) {
		me._type = "multi";
		//标记特殊覆盖物，如果小区标记为真，聚合点标记改为真
		if(gisCell.thFlag==true){
			me._thFlag=true;
		}
	} else {
		// 显示用的经纬度
		...
		//标记特殊覆盖物
		me._thFlag = (typeof gisCell.thFlag!="undefined")?gisCell.thFlag:"";
