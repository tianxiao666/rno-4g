...
function bindEvent(){
	$("#importBtn").click(function() {
		...
		$("#err").remove();
		if ($("#fileid").val() == "") {
			$("#fileid").parent().append("<font id='err' color='red'>请选择话统文件</font>");
			return;
		}
		/*if ($.trim($("#meatime").val()) == "") {
...
/*		var factory=$("#formImportData #factory").find("option:selected").val();
	...
	$("#meaBegDate").datetimepicker(
			{
				...
				numberOfMonths : 1,
				onClose : function(selectedDate) {
					$("#meaEndDate").datetimepicker("option", "minDate",
							selectedDate);
				}
			});
	$("#meaBegDate").datetimepicker("setDate",addDays(new Date(),-2));// 减去2天
	$("#meaEndDate").datetimepicker(
			{
				dateFormat : "yy-mm-dd",
				timeFormat: "HH:mm:ss",
				defaultDate :"+1w",
				...
				numberOfMonths : 1,
				onClose : function(selectedDate) {
					$("#meaBegDate").datetimepicker("option", "maxDate",
							selectedDate);
				}
			});
	$("#meaEndDate").datetimepicker("setDate",(new Date()));
...
function queryImportDataRec(){
	$("#searchImportForm").ajaxSubmit({
		...
		success:function(raw){
			...
			setPageView(data['page'],"ImportRecPageDiv");
		}
	});
}
...
/*	var factory=$("#formImportData #factory").find("option:selected").val();
	$("#formImportData #fileCode").val(factory);*/
	$("#progressNum").text('0%');
	$("#progressbar").progressbar({
		value : 0
	});
	...
	fileSize = 1;
	var fileCode = $("#formImportData #fileCode").val();
	var attachParams = {};
	attachParams['cityId'] = $("#cityId1").val();
	/*console.log($("#formImportData #areaType").find("option:selected").val());
	attachParams['areaType'] = $("#formImportData #areaType").find("option:selected").val();*/
	//attachParams['areaType']=$("#formImportData #areaType").find("option:selected").val();
	// 询问是否可以上传
	$.ajax({
 ...
 * 查询数据描述数据
 */
function queryDataDescData(){
	$("#searchDataForm").ajaxSubmit({
		url:'queryG4StsDataAjaxAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
			...
			displayDataDescData(data['data']);
			setFormPageInfo("searchDataForm",data['page']);
			setPageView(data['page'],"dataListPageDiv");
		}
	});
}
 ...
 * @param data
 */
function displayDataDescData(data){
	if(data==null||data==undefined){
		return;
	}
	//
	$("#dataListTab tr:not(:first)").each(function(i, ele) {
		$(ele).remove();
	});
	var html="";
	...
	for ( var i = 0; i < data.length; i++) {
		...
		html+= "<tr>";
		html+="<td>"+city+"</td>";
		html+="<td>"+getValidValue(one['BEGINTIME'],'')+"</td>";
		html+="<td>"+getValidValue(one['ENDTIME'],'')+"</td>";
		html+="<td>"+getValidValue(one['INFOMODELREFERENCED'],'')+"</td>";
		html+="<td>"+getValidValue(one['DNPREFIX'],'')+"</td>";
		html+="<td>"+getValidValue(one['SENDERNAME'],'')+"</td>";
		html+="<td>"+getValidValue(one['VENDORNAME'],'')+"</td>";
		html+="<td>"+getValidValue(one['JOBID'],'')+"</td>";
		html+="<td>"+getValidValue(one['CNT'],'')+"</td>";
		html+="<td>"+getValidValue(one['CREATE_TIME'],'')+"</td>";
		html+="</tr>";
	}
	html+="</tbody>";
	$("#dataListTab").append(html);
}
