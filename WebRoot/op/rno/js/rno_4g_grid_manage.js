...
function queryNcsDescData(){
	$("#searchNcsForm").ajaxSubmit({
		url:'queryLteGridDataByPageAjaxAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
...
function displayNcsDescData(data){
	...
	for ( var i = 0; i < data.length; i++) {
		...
		html += "<tr>";
		html+="<td>"+city+"</td>";
		html+="<td>"+getValidValue(one['GRID_ID'],'')+"</td>";
		html+="<td>"+getValidValue(one['GRID_DESC'],'')+"</td>";
		html+="<td>"+getValidValue(one['GRID_CENTER'],'')+"</td>";
