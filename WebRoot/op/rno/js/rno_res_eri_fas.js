...
function bindEvent(){
	$("#importBtn").click(function() {
		if(!(fileid.value.toUpperCase().endsWith(".CSV")||fileid.value.toUpperCase().endsWith(".XLS")
				||fileid.value.toUpperCase().endsWith(".XLSX")||fileid.value.toUpperCase().endsWith(".XML")
				||fileid.value.toUpperCase().endsWith(".TXT")||fileid.value.toUpperCase().endsWith(".ZIP"))){
			alert("不支持该类型文件！");
			return false;
		}	
		...
		$("#err").remove();
		if ($("#fileid").val() == "") {
			$("#fileid").parent().append("<font id='err' color='red'>请选择爱立信fas文件</font>");
			return;
		}
		if ($.trim($("#meatime").val()) == "") {
	...
	//查询爱立信fas描述数据
	$("#searchFasBtn").click(function(){
		initFormPage("searchFasForm");
		queryFasDescData();
	});
...
function jqueryUiSet() {
	...
	$("#fasMeaBegDate").datetimepicker("setDate",addDays(new Date(),-2));// 减去2天
...
function queryImportDataRec(){
	$("#searchImportForm").ajaxSubmit({
		...
		success:function(raw){
			...
			setPageView(data['page'],"fasImportRecPageDiv");
		}
	});
}
 ...
 * 查询爱立信fas描述数据
 */
function queryFasDescData(){
	$("#searchFasForm").ajaxSubmit({
		url:'queryEriFasDescAjaxAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
			...
			displayFasDescData(data['data']);
			setFormPageInfo("searchFasForm",data['page']);
			setPageView(data['page'],"fasListPageDiv");
		}
	});
}
 ...
 * @param data
 */
function displayFasDescData(data){
	if(data==null||data==undefined){
		return;
	}
	//
	$("#fasListTab tr:not(:first)").each(function(i, ele) {
		$(ele).remove();
	});
	var html="";
